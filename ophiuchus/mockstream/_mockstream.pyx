...
from ._coord cimport sat_rotation_matrix, _to_sat_coords, _from_sat_coords
...
cdef void car_to_cyl(double *xyz, double *cyl):
    cdef:
...
cdef void cyl_to_car(double *cyl, double *xyz):
...
cdef void v_car_to_cyl(double *xyz, double *vxyz, double *vcyl):
    cdef:
        double R = sqrt(xyz[0]*xyz[0]+xyz[1]*xyz[1])
        double vR = (xyz[0]*vxyz[0] + xyz[1]*vxyz[1]) / R
        double vphi = (xyz[0]*vxyz[1] - vxyz[0]*xyz[1]) / R

    vcyl[0] = vR
    vcyl[1] = vphi
    vcyl[2] = vxyz[2]

cdef void v_cyl_to_car(double *xyz, double *vcyl, double *vxyz):
    cdef double R = sqrt(xyz[0]*xyz[0]+xyz[1]*xyz[1])

    vxyz[0] = vcyl[0] * xyz[0]/R - xyz[1]/R * vcyl[1]
    vxyz[1] = vcyl[0] * xyz[1]/R + xyz[0]/R * vcyl[1]
    vxyz[2] = vcyl[2]
